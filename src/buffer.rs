...
impl Buffer {
    pub fn from(s: &str) -> Self {
        ...
        Buffer { lines, cursor_x, cursor_y, reminder_cursor_x }
    }
    ...
    pub fn move_cursor_vertical(&mut self, y: i64) {
        ...
        let length_of_line = UnicodeSegmentation::graphemes(self.lines[self.cursor_y].as_str(), true).count();
        if length_of_line == 0 {
            self.cursor_x = 0;
        } else {
    ...
    pub fn get_under_cursor(&self) -> &str {
        ...
        UnicodeSegmentation::graphemes(self.lines[self.cursor_y].as_str(), true).nth(self.cursor_x).unwrap()
    }
    pub fn insert_under_cursor(&mut self, s: &str) {
        debug_assert!(self.cursor_y < self.lines.len());
        let line_graphemes = UnicodeSegmentation::graphemes(self.lines[self.cursor_y].as_str(), true).collect::<Vec<_>>();
        if s == "\n" {
            ...
            self.lines[self.cursor_y] = line_graphemes[..self.cursor_x].join("") + s + &line_graphemes[self.cursor_x..].join("");
    ...
    pub fn delete_under_cursor(&mut self) {
        ...
        if self.cursor_x > 0 {
            ...
            if self.cursor_y > 0 {
                let previous_line_len = UnicodeSegmentation::grapheme_indices(self.lines[self.cursor_y-1].as_str(), true).count();
                self.lines[self.cursor_y] = self.lines[self.cursor_y-1].clone() + &self.lines[self.cursor_y];
